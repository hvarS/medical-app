...
from anti_celiac.__init__ import PY2
from anti_celiac.__init__ import CALIBRATE

import anti_celiac.utils as utils
from anti_celiac.config import get_config
from anti_celiac.label_file import LabelFile
from anti_celiac.label_file import LabelFileError
from anti_celiac.logger import logger
from anti_celiac.shape import Shape
from anti_celiac.widgets import BrightnessContrastDialog
from anti_celiac.widgets import Canvas
from anti_celiac.widgets import FileDialogPreview
from anti_celiac.widgets import LabelDialog
from anti_celiac.widgets import LabelListWidget, UniqueLabelListWidget
from anti_celiac.widgets import LabelListWidgetItem
from anti_celiac.widgets import ToolBar
from anti_celiac.widgets import UniqueLabelQListWidget
from anti_celiac.widgets import ZoomWidget
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(
        ...
        ## Addition: To Check whether IR Mode has been activated or not
        ...
        ## Addition: Initialise a unique label list for on and off display
        self.uniqLabelListCheckable = UniqueLabelListWidget()
        self.uniqLabelListCheckable.itemSelectionChanged.connect(self.labelSelectionChanged)
        self.uniqLabelListCheckable.itemDoubleClicked.connect(self.editLabel)
        self.uniqLabelListCheckable.itemChanged.connect(self.uniqueLabelItemChanged)
        self.uniqLabelListCheckable.itemDropped.connect(self.labelOrderChanged)
        ##
        
        self.labelList = LabelListWidget()
        self.lastOpenDir = None
        
        ## Addition: Creating a dock for displaying counts and calculations for AI Actions
        ...
        # Addition : Changing label dock widget to unique Label List Checkable
        # self.label_dock.setWidget(self.uniqLabelList)
        self.label_dock.setWidget(self.uniqLabelListCheckable)
        ...
        self.canvas.newIR.connect(self.newIR)
        ...
        ######### Adding Segmentation And IELs Counting , Important Tissue, Button to app ###########
        ...
        tissueMode = action(
            ...
            self.tissueMode,
            shortcut = shortcuts["seg_tissue"],
            icon = "seg_tissue",
            tip = self.tr("Segment the important tissues"),
            enabled=False,
        )

        interpretableRegionMode = action(
            self.tr("&Interpretable Region"),
            lambda: self.interpretableRegionMode(False, createMode="rectangle"),
            shortcuts["interpretable"],
            "crop",
            self.tr("Start Cropping for Correct Identification"),
            enabled=False,
        )

        lengthMeasurementMode = action(
            self.tr("&Calibration"),
            lambda: self.lengthMeasurementMode(createMode="linestrip"),
            shortcuts["lenMes"],
            "lenMes",
            self.tr("Draw a Line to Measure the length"),
            enabled=False,
        )

        actualMeasurement = action(
            self.tr("&Measure"),
            lambda: self.actualMeasurement(),
            shortcuts["calcMes"],
            "calcMes",
        ...
        recalibrationMode = action(
            ...
            lambda: self.recalibrationMode(),
            shortcuts["recal"],
            "calcMes",
            self.tr("Recalibrate based on a custom scale"),
            enabled=False,
        )
        ...
        self.actions = utils.struct(
            ...
            saveAs=saveAs,
            open=open_,
            lengthMeasurementMode = lengthMeasurementMode,
        ...
        self.menus = utils.struct(
            ...
            measurementAndLengthActions = self.menu(self.tr("&Calibration/Measurement")), ### Adding Calibration Menu
        ...
        utils.addActions(
            self.menus.measurementAndLengthActions,
            (
                lengthMeasurementMode,
                actualMeasurement,
                recalibrationMode,
    ...
    def popup_recalibration_dialog(self, distpix):
        ...
        height, width = pmap.height(), pmap.width()
        recal_ui = calibrationSet(Select,distpix,[height,width])
        recal_ui.setupUi()
        Select.exec_()
        self.CUSTOM_CALIBRATION = recal_ui.custom_calibration
    ...
    def selectCalibrationOption(self):
        self.mode = 'CALPOPUP'
        Select = QtWidgets.QDialog()
        ui = calibrationSelection(self,Select)
        ui.setupUi()
        Select.exec_()
        self.CALIBRATION_INDEX = ui.CALIBRATION_INDEX
    ...
    def actualMeasurement(self):
        ...
        for shape in self.canvas.shapes:
            ...
            for idx in range(1,len(points)):
                cur_point = int(points[idx].x()),int(points[idx].y())
                dist += math.sqrt((cur_point[0]-prev_point[0])**2+(cur_point[1]-prev_point[1])**2)
                prev_point = cur_point

            if self.CALIBRATION_INDEX!=-1:
                CALIBRATION_CONSTANT = CALIBRATE[self.CALIBRATION_INDEX]
            else:
    ...
    def get_trimask(self):
        ...
        trimask_save_path = os.path.join(os.getcwd(),'temp_saver','rgb_trimask',img_name)

        if not os.path.exists(trimask_save_path):
            os.makedirs(trimask_save_path)

        crypts_mask = os.path.join(mask_root,'Crypts')
        villi_mask = os.path.join(mask_root,'Villi')
        epithelium_mask = os.path.join(mask_root,'Epithelium')
        brunner_mask = os.path.join(mask_root,'Brunner\'s Gland')

        filename = img_name+'.png'
        w,h = self.canvas.pixmap.width(), self.canvas.pixmap.height()
        rgb_mask = np.zeros((h, w, 3))
        rgb_mask[:, :, 0] += np.array(cv2.imread(os.path.join(crypts_mask, filename),cv2.IMREAD_GRAYSCALE)) / 255
        rgb_mask[:, :, 1] += np.array(cv2.imread(os.path.join(villi_mask, filename),cv2.IMREAD_GRAYSCALE)) / 255
        rgb_mask[:, :, 2] += np.array(cv2.imread(os.path.join(epithelium_mask, filename),cv2.IMREAD_GRAYSCALE)) / 255
        rgb_mask[:, :, :] += np.expand_dims(np.array(cv2.imread(os.path.join(brunner_mask, filename),cv2.IMREAD_GRAYSCALE)), -1) * 0.5 / 255.
        rgb_mask = np.clip(rgb_mask, 0., 1.)
        rgb_mask = np.uint8(rgb_mask * 255)

        img_save_loc = os.path.join(trimask_save_path,img_name+'.png')
        cv2.imwrite(img_save_loc,rgb_mask)
    ...
    def getCroppedImage(self):
        ...
        for shape in self.canvas.shapes:
            label = shape.label
            if label in ['IR','Interpretable Region']:
                points = shape.points
                ul, br = points
                break
        
        ul_x , ul_y , br_x, br_y = round(ul.x()), round(ul.y()), round(br.x()), round(br.y())
        cropped_img = img[ul_y:br_y,ul_x:br_x]

        img_write_path = os.path.join(temp_save_dir,img_name+'.png')
        cv2.imwrite(img_write_path,cropped_img)
    ...
    def get_corrected_interpretable_image_mask(self):
        ...
        for shape in self.canvas.shapes:
            ...
            cv2.fillPoly(mask_array, pts =[np.array(contour)], color=(255,255,255))

        mask_array[mask_array == 255] = 1
        selected_mask = np.expand_dims(mask_array, axis=-1)



        interpretable_img = img*selected_mask
        img_write_path = os.path.join(temp_save_dir,img_name+'.png')
    ...
    def fillStatsAfterCounting(self):
        ...
        for shape in self.canvas.shapes:
            lbl = shape.label
            if lbl not in self.statsDict:
                self.statsDict[lbl] = 1
            else:
                self.statsDict[lbl] += 1

    def calculateQHistScore(self):
        try:
    ...
    def addLabel(self, shape):
        ...
        if self.uniqLabelList.findItemByLabel(shape.label) is None:
            ...
            ## Addition: Adding only the unique item 
            # unique_label_list_item = LabelListWidgetItem(text, shape)
            unique_label_list_item = LabelListWidgetItem(shape = shape)
            
            ## Changing the size of the labelItem
            unique_label_list_item.setText(
                '<h2><font color="#{:02x}{:02x}{:02x}" size="">{} ●</font></h2>'.format(
                    *rgb, html.escape(text.split('(')[0].strip())
                )
            )

            self.uniqLabelListCheckable.addItem(unique_label_list_item)
            ##
        ...
        label_list_item.setText(
            '<p>{} <font color="#{:02x}{:02x}{:02x}" size="4">●</font></p>'.format(
    ...
    def loadShapes(self, shapes, replace=True):
        ...
        for shape in shapes:
            self.addLabel(shape)
        self.labelList.clearSelection()
        self.uniqLabelListCheckable.clearSelection()
        self._noSelectionSlot = True
    ...
    def loadLabels(self, shapes):
        ...
        for shape in shapes:
            ...
            ## TODO: Correcting Group Id loading
    ...
    def uniqueLabelItemChanged(self, unique_item):
        """ The Item Here is ListWidgetItem """
        shape = unique_item.shape()
        value = unique_item.checkState()
        all_items_for_the_unique_labels = self.labelList.findAllItemsByShapeLabel(shape.label)
        for item in all_items_for_the_unique_labels:
            individual_shape = item.shape()
            item.setCheckState(Qt.Checked if value else Qt.Unchecked)
            self.canvas.setShapeVisible(individual_shape, item.checkState() == Qt.Checked)
    ...
    def newIR(self):
        """
            Set the label for cropped IR section
        """
    ...
    def newShape(self):
        ...
        """
        if self.mode == 'RECAL':
            points = self.canvas.line.points
            p1 = int(points[0].x()),int(points[0].y())
            p2 = int(points[1].x()),int(points[1].y())
            distpix = round(math.sqrt(math.pow(p1[0]-p2[0],2)+math.pow(p1[1]-p2[1],2)))
            self.popup_recalibration_dialog(distpix)
        else:
    ...
    """Addition: Function to toggle the unique label list"""
    def toggleUniqueLabels(self,boolean):
        for item in self.uniqLabelListCheckable:
            item.setCheckState(Qt.Checked if boolean else Qt.Unchecked)
