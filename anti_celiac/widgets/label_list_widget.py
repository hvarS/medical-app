...
class LabelListWidget(QtWidgets.QListView):
    ...
    def addItem(self, item):
        ...
        item.setSizeHint(QtCore.QSize(400,400))
    ...
    def findAllItemsByShapeLabel(self,label):
        items = []
        for row in range(self.model().rowCount()):
            item = self.model().item(row, 0)
            if item.shape().label == label:
                items.append(item)
        return items
...
class UniqueLabelListWidget(QtWidgets.QListView):

    itemDoubleClicked = QtCore.Signal(LabelListWidgetItem)
    itemSelectionChanged = QtCore.Signal(list, list)

    def __init__(self):
        super(UniqueLabelListWidget, self).__init__()
        self._selectedItems = []
    ...
    def addItem(self, item):
        if not isinstance(item, LabelListWidgetItem):
            raise TypeError("item must be LabelListWidgetItem")
        self.model().setItem(self.model().rowCount(), 0, item)
        item.setSizeHint(self.itemDelegate().sizeHint(400, 400))
