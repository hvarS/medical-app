...
from anti_celiac import __version__
from anti_celiac.logger import logger
from anti_celiac import PY2
...
class LabelFile(object):
    ...
    def load_image_file(filename):
        ...
        if ext in ['tiff','tif']:
            np_array = tiff_parser(filename,0, 0, 1000, 1000, 0)

        try:
            if ext not in ['tiff','tif']:
                image_pil = PIL.Image.open(filename)
            else:
                image_pil = PIL.Image.fromarray(np_array.astype('uint8'), 'RGB')
